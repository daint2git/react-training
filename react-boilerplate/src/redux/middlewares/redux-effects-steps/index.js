 ...
 * Middleware
 */
export function stepsMiddleware({ dispatch }) {
  ...
  function dispatchEffect(action) {
    ...
    return action.meta && action.meta.steps ? applySteps(promise, action.meta.steps) : promise
  }
  ...
  function applySteps(initialPromise, stepArray = []) {
    return stepArray
      ...
      .reduce(
        (promise, [success = noop, failure = reject]) =>
          promise.then(
